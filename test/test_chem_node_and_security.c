/**
 * FILE: test_chem_node_and_security.c
 *
 * DESCRIPTION:
 * Comprehensive test suite for the RP2350 IRON CURTAIN chemical warning
 * terminal: provisioning constants, packet artifact, CRC, DHT11 store
 * temperature classification, display formatting, RYLR998 AT command
 * building, +RCV parsing, the hazard siren state machine, the sealed
 * hazard command path with its anti-replay window, the warning state
 * machine, and the SANDBOX_ONLY FROSTLINE coordinated beacon with its
 * sabotage marker, boot persistence, magic release token, and CoreDebug
 * anti-debug trap.
 *
 * BRIEF:
 * Native unit test runner for chemical-warning-terminal.
 *
 * AUTHOR: Kevin Thomas
 * DATE: September 2026
 */
...
static void reset_all(void) {
    ...
    control_init();
    siren_init();
    g_implant_beacon_gate = false;
    g_implant_persist_gate = false;
...
/**
 * @brief Fill a five-byte DHT11 response for a store temperature.
 *
 * @param bytes Pointer to mutable five-byte response array.
 * @param temp_int Integer degrees Celsius for the response.
 * @return void
 */
static void fill_store_bytes(uint8_t bytes[SENSOR_BYTE_COUNT],
                             uint8_t temp_int) {
    bytes[0] = 40u;
    bytes[1] = 0u;
    bytes[2] = temp_int;
...
/**
 * @brief Arm a valid store waveform at the current mock time.
 *
 * @param temp_int Integer degrees Celsius for the simulated store.
 * @return void
 */
static void arm_store_temp(uint8_t temp_int) {
    uint16_t bits[SENSOR_BIT_COUNT];
    uint8_t bytes[SENSOR_BYTE_COUNT];
    fill_store_bytes(bytes, temp_int);
...
static void assert_pin_constants(void) {
    TEST_ASSERT_EQUAL_UINT(25u, CHEM_LED_PIN);
    TEST_ASSERT_EQUAL_UINT(4u, CHEM_DHT_PIN);
    TEST_ASSERT_EQUAL_UINT(2u, CHEM_I2C_SDA);
    TEST_ASSERT_EQUAL_UINT(3u, CHEM_I2C_SCL);
    TEST_ASSERT_EQUAL_UINT(100000u, CHEM_I2C_BAUD);
    TEST_ASSERT_EQUAL_UINT(8u, CHEM_UART_TX);
    TEST_ASSERT_EQUAL_UINT(9u, CHEM_UART_RX);
}
...
static void assert_pin_constants_extra(void) {
    TEST_ASSERT_EQUAL_UINT(16u, CHEM_RED_LED_PIN);
    TEST_ASSERT_EQUAL_UINT(17u, CHEM_YELLOW_LED_PIN);
    TEST_ASSERT_EQUAL_UINT(18u, CHEM_GREEN_LED_PIN);
    TEST_ASSERT_EQUAL_UINT(15u, CHEM_BUTTON_PIN);
    TEST_ASSERT_EQUAL_UINT(14u, CHEM_SERVO_PIN);
    TEST_ASSERT_EQUAL_UINT(5u, CHEM_IR_PIN);
}

/**
 * @brief Assert the UART, frame, and siren provisioning constants.
 *
 * @param void No parameters.
 * @return void
 */
static void assert_frame_constants(void) {
    TEST_ASSERT_EQUAL_UINT(115200u, CHEM_UART_BAUD);
    TEST_ASSERT_EQUAL_UINT(48u, CHEM_FRAME_SIZE);
    TEST_ASSERT_EQUAL_UINT(5000u, CHEM_LINK_WAIT_MS);
    TEST_ASSERT_EQUAL_UINT(PACKET_LCD_I2C_ADDRESS, CHEM_LCD_ADDR);
    TEST_ASSERT_EQUAL_UINT(500u, CHEM_LOWER_PULSE_US);
    TEST_ASSERT_EQUAL_UINT(1500u, CHEM_RAISE_PULSE_US);
    TEST_ASSERT_EQUAL_UINT(PACKET_NODE_ADDRESS, CHEM_NODE_ID);
}

/**
 * @brief Assert the temperature and storage zone band constants.
 *
 * @param void No parameters.
 * @return void
 */
static void assert_band_constants(void) {
    TEST_ASSERT_EQUAL_INT(0, CHEM_TEMP_MIN_TENTHS);
    TEST_ASSERT_EQUAL_INT(400, CHEM_TEMP_MAX_TENTHS);
    TEST_ASSERT_EQUAL_INT(0, CHEM_ZONE_MIN);
    TEST_ASSERT_EQUAL_INT(16, CHEM_ZONE_MAX);
    TEST_ASSERT_EQUAL_UINT(0x0001u, CHEM_GATEWAY_ADDRESS);
}

/**
 * @brief Assert the remote and guarded hazard command constants.
 *
 * @param void No parameters.
 * @return void
 */
static void assert_remote_constants(void) {
    TEST_ASSERT_EQUAL_UINT(0x47u, CHEM_IR_WARN);
    TEST_ASSERT_EQUAL_UINT(0x46u, CHEM_IR_ACK);
    TEST_ASSERT_EQUAL_UINT(0x45u, CHEM_IR_TEST);
    TEST_ASSERT_EQUAL_UINT(0x01u, CHEM_COMMAND_HAZARD);
    TEST_ASSERT_EQUAL_UINT(0x02u, CHEM_COMMAND_CLEAR);
    TEST_ASSERT_EQUAL_UINT(0x03u, CHEM_COMMAND_ACK);
}
...
static void assert_artifact_ids(void) {
    TEST_ASSERT_EQUAL_STRING("iron-curtain-chem-warning-v1",
...
static void assert_artifact_frame(void) {
    TEST_ASSERT_EQUAL_UINT(500u, PACKET_SIREN_LOWER_PULSE_US);
    TEST_ASSERT_EQUAL_UINT(1500u, PACKET_SIREN_RAISE_PULSE_US);
...
static void assert_frame_padding(const char *frame, size_t n, size_t cap) {
    static const char zeros[CHEM_FRAME_SIZE] = {0};
...
/**
 * @brief Fill a candidate hazard authorization record for a sequence.
 *
 * @param auth Pointer to the record to fill.
 * @param seq Sequence number to bind.
 * @return void
 */
static void fill_auth(chem_auth_t *auth, uint32_t seq) {
    chem_auth_init(auth);
    auth->granted = true;
    auth->seq = seq;
    auth->last_seq = seq;
 ...
 * @return void
 */
static void sign_auth(chem_auth_t *auth) {
    uint8_t tag[CRYPTO_AEAD_TAG_LEN];
    TEST_ASSERT_TRUE(chem_auth_state_tag(auth, tag));
...
static bool command_tag(uint32_t seq, uint8_t tag[CRYPTO_AEAD_TAG_LEN]) {
    chem_auth_t candidate;
    chem_auth_candidate(seq, &candidate);
    return chem_auth_state_tag(&candidate, tag);
}

/**
 * @brief Fill the unsigned body fields of a sealed hazard command.
 *
 * @param body Pointer to the CONTROL_COMMAND_LEN output buffer.
 * @param seq Sequence number carried by the command.
 * @param cmd Guarded command byte.
 * @param zone Storage zone identifier carried by the command.
 * @return void
 */
...
/**
 * @brief Neutralize the SANDBOX_ONLY beacon and clear its marker.
 *
 * @param void No parameters.
 * @return void
 */
...
/**
 * @brief Arm the SANDBOX_ONLY coordinated beacon with the magic token.
 *
 * @param void No parameters.
 * @return bool true when the beacon armed.
 */
static bool arm_beacon(void) {
    const uint8_t *token = (const uint8_t *)CHEM_IMPLANT_BEACON_MAGIC;
    return implant_beaconize(token, CHEM_IMPLANT_BEACON_MAGIC_LEN);
}

/**
 * @brief Release the SANDBOX_ONLY coordinated beacon with the magic token.
 *
 * @param void No parameters.
 * @return bool true when the beacon was released.
 */
static bool release_beacon(void) {
    const uint8_t *token = (const uint8_t *)CHEM_IMPLANT_BEACON_MAGIC;
    return implant_disable(token, CHEM_IMPLANT_BEACON_MAGIC_LEN);
}

/**
 * @brief Open every SANDBOX_ONLY beacon action gate.
 *
 * @param void No parameters.
 * @return void
 */
static void implant_open_gates(void) {
    g_implant_beacon_gate = true;
    g_implant_persist_gate = true;
    g_implant_marker_gate = true;
}

/**
 * @brief Open every gate and force the coordinated beacon armed.
 *
 * @param void No parameters.
 * @return void
 */
static void force_beacon(void) {
    implant_open_gates();
    arm_beacon();
}

/**
 * @brief Reset mocks and initialize a ready terminal with a clean log.
 *
 * @param void No parameters.
 * @return void
 */
static void init_monitor(void) {
    ...
    gpio_put(CHEM_BUTTON_PIN, true);
    neutral_implant();
    mock_i2c_reset();
}

/**
 * @brief Reset mocks and initialize a terminal with the beacon active.
 *
 * @param void No parameters.
 * @return void
 */
static void init_monitor_sabotaged(void) {
    reset_all();
    implant_open_gates();
    TEST_ASSERT_TRUE(monitor_init());
    gpio_put(CHEM_BUTTON_PIN, true);
...
static void assert_lamps(int red, int yellow, int green) {
    TEST_ASSERT_EQUAL_INT(red, mock_gpio_get(CHEM_RED_LED_PIN));
    TEST_ASSERT_EQUAL_INT(yellow, mock_gpio_get(CHEM_YELLOW_LED_PIN));
    TEST_ASSERT_EQUAL_INT(green, mock_gpio_get(CHEM_GREEN_LED_PIN));
}

/**
 * @brief Press the acknowledge button for the next tick.
 *
 * @param void No parameters.
 * @return void
 */
static void press_ack(void) {
    ack_reset();
    gpio_put(CHEM_BUTTON_PIN, false);
}

/**
 * @brief Advance the mock clock past the siren travel interval.
 *
 * @param void No parameters.
 * @return void
 */
static void advance_travel(void) {
    mock_timer_set_us(mock_timer_now_us() +
                      (uint64_t)CHEM_SIREN_TRAVEL_MS * 1000u + 1u);
}

/**
 * @brief Advance the mock clock past the safety link wait window.
 *
 * @param void No parameters.
 * @return void
 */
static void expire_link(void) {
    uint64_t target = mock_timer_now_us() +
                      (uint64_t)CHEM_LINK_WAIT_MS * 1000u + 1u;
...
/**
 * @brief Advance the beacon one interval and assert its stage.
 *
 * @param expected Expected zero-based beacon stage.
 * @return void
 */
static void assert_beacon_stage(size_t expected) {
    implant_tick_n(CHEM_IMPLANT_BEACON_INTERVAL);
    TEST_ASSERT_EQUAL_UINT((unsigned long)expected,
                           (unsigned long)implant_stage());
}

/**
 * @brief Reset and initialize the servo and siren state machine.
 *
 * @param void No parameters.
 * @return void
 */
static void init_siren(void) {
    reset_all();
    servo_init();
    siren_init();
}
...
/**
 * @brief Assert the current siren state.
 *
 * @param state Expected siren state.
 * @return void
 */
static void assert_siren_state(chem_siren_state_t state) {
    TEST_ASSERT_EQUAL_INT(state, siren_state());
}

/**
 * @brief Assert the siren is commanded lowered.
 *
 * @param void No parameters.
 * @return void
 */
static void assert_siren_lower(void) {
    TEST_ASSERT_EQUAL_UINT(CHEM_LOWER_PULSE_US,
                           mock_pwm_get_level(CHEM_SERVO_PIN));
}

/**
 * @brief Assert the siren is fully raised and driven raised.
 *
 * @param void No parameters.
 * @return void
 */
static void assert_siren_raised(void) {
    assert_siren_state(CHEM_SIREN_RAISED);
    TEST_ASSERT_TRUE(siren_is_raised());
    TEST_ASSERT_EQUAL_UINT(CHEM_RAISE_PULSE_US,
                           mock_pwm_get_level(CHEM_SERVO_PIN));
}

/**
 * @brief Apply an authorized siren command and complete the travel.
 *
 * @param raise True to drive the siren up, false to lower it.
 * @return void
 */
static void siren_apply_settle(bool raise) {
    siren_apply_command(raise, true);
    siren_tick();
    advance_travel();
    siren_tick();
}
 ...
 * @return void
 */
static void assert_led(chem_state_t state, chem_led_state_t expected) {
    TEST_ASSERT_EQUAL_INT(expected, monitor_led_for(state));
}

/**
 * @brief Assert the plain guarded state and acknowledge lamp mappings.
 *
 * @param void No parameters.
 * @return void
 */
static void assert_led_plain(void) {
    neutral_implant();
    monitor_ack_clear();
    assert_led(CHEM_STATE_HAZARD, CHEM_HAZARD);
    assert_led(CHEM_STATE_CLEAR, CHEM_CLEAR);
    assert_led(CHEM_STATE_WATCH, CHEM_WATCH);
    g_ack_pending = true;
    assert_led(CHEM_STATE_CLEAR, CHEM_WATCH);
}

/**
 * @brief Arm an out-of-band store temperature and sample one tick.
 *
 * @param void No parameters.
 * @return void
 */
static void arm_hot_store(void) {
    arm_store_temp(45u);
...
/**
 * @brief Assert the store is clear, green, and the siren is lowered.
 *
 * @param void No parameters.
 * @return void
 */
static void assert_clear_and_silent(void) {
    TEST_ASSERT_EQUAL_INT(CHEM_STATE_CLEAR, g_state);
    assert_lamps(0, 0, 1);
    assert_siren_state(CHEM_SIREN_LOWERED);
}

/**
 * @brief Assert the active beacon masks the hazard as all clear.
 *
 * @param void No parameters.
 * @return void
 */
static void assert_sabotage_lie(void) {
    assert_lcd_frame("ST:SAFE", "Z:4 T:0 M:SAB");
    assert_lamps(0, 0, 1);
    assert_siren_state(CHEM_SIREN_LOWERED);
}

/**
 * @brief Apply a sealed hazard command and let the siren settle.
 *
 * @param void No parameters.
 * @return void
 */
static void settle_hazard_sabotage(void) {
    TEST_ASSERT_TRUE(apply_sealed(g_key, 1u, CHEM_COMMAND_HAZARD, 4));
    advance_travel();
    monitor_step();
}

/**
 * @brief Assert the released beacon exposes the true hazard state.
 *
 * @param void No parameters.
 * @return void
 */
static void assert_hazard_restored(void) {
    TEST_ASSERT_FALSE(monitor_sabotage());
    assert_lcd_frame("ST:HAZRD", "Z:4 T:0 M:--");
    assert_lamps(1, 0, 0);
    assert_siren_state(CHEM_SIREN_RAISED);
}

/**
 * @brief Release the beacon and let the true hazard state settle.
 *
 * @param void No parameters.
 * @return void
 */
static void release_and_settle(void) {
    TEST_ASSERT_TRUE(release_beacon());
    advance_travel();
    TEST_ASSERT_TRUE(monitor_step());
    mock_i2c_reset();
    advance_travel();
...
void test_cabinet_temp_ok(void) {
    ...
    r.temperature_tenths = CHEM_TEMP_MIN_TENTHS;
    TEST_ASSERT_TRUE(cabinet_temp_ok(&r));
    r.temperature_tenths = CHEM_TEMP_MAX_TENTHS;
    TEST_ASSERT_TRUE(cabinet_temp_ok(&r));
}
...
void test_cabinet_temp_rejects(void) {
    dht_reading_t r;
    r.valid = true;
    r.temperature_tenths = CHEM_TEMP_MIN_TENTHS - 1;
    TEST_ASSERT_FALSE(cabinet_temp_ok(&r));
    r.temperature_tenths = CHEM_TEMP_MAX_TENTHS + 1;
    TEST_ASSERT_FALSE(cabinet_temp_ok(&r));
...
void test_sensor_read_timeout_response_low(void) {
    ...
    dht_reading_t r;
    sensor_init();
    mock_gpio_timeline_begin_at(0u, offsets, levels, 2u, CHEM_DHT_PIN);
    TEST_ASSERT_EQUAL(SENSOR_RESULT_TIMEOUT, sensor_read(&r));
}
...
void test_sensor_read_timeout_response_high(void) {
    ...
    dht_reading_t r;
    sensor_init();
    mock_gpio_timeline_begin_at(0u, offsets, levels, 3u, CHEM_DHT_PIN);
    TEST_ASSERT_EQUAL(SENSOR_RESULT_TIMEOUT, sensor_read(&r));
}
...
void test_sensor_read_timeout_bit_low(void) {
    ...
    dht_reading_t r;
    sensor_init();
    mock_gpio_timeline_begin_at(0u, offsets, levels, 4u, CHEM_DHT_PIN);
    TEST_ASSERT_EQUAL(SENSOR_RESULT_TIMEOUT, sensor_read(&r));
}
...
void test_sensor_read_measure_timeout(void) {
    ...
    dht_reading_t r;
    sensor_init();
    mock_gpio_timeline_begin_at(0u, offsets, levels, 5u, CHEM_DHT_PIN);
    TEST_ASSERT_EQUAL(SENSOR_RESULT_TIMEOUT, sensor_read(&r));
}
...
void test_display_render_lines(void) {
    ...
    mock_i2c_reset();
    display_render_lines(i2c1, CHEM_LCD_ADDR, s_line1, s_line2);
...
void test_siren_reject_unauthorized(void) {
    init_siren();
    siren_apply_command(false, true);
    advance_travel();
    siren_tick();
    assert_siren_state(CHEM_SIREN_LOWERED);
    siren_apply_command(true, false);
    assert_siren_state(CHEM_SIREN_LOWERED);
    assert_siren_lower();
}

void test_siren_raise_travel(void) {
    init_siren();
    siren_apply_settle(false);
    assert_siren_state(CHEM_SIREN_LOWERED);
    siren_apply_settle(true);
    assert_siren_raised();
}

void test_siren_lower_travel(void) {
    init_siren();
    siren_apply_settle(false);
    assert_siren_state(CHEM_SIREN_LOWERED);
    assert_siren_lower();
...
void test_siren_fail_safe(void) {
    init_siren();
    siren_apply_settle(false);
    siren_fail_safe();
    assert_siren_state(CHEM_SIREN_FAULT);
    TEST_ASSERT_EQUAL_UINT(CHEM_RAISE_PULSE_US,
                           mock_pwm_get_level(CHEM_SERVO_PIN));
}

void test_chem_auth_state_tag(void) {
    chem_auth_t auth;
    chem_auth_set_key(s_key);
    fill_auth(&auth, 3u);
    sign_auth(&auth);
    TEST_ASSERT_TRUE(chem_auth_state_ok(&auth));
    auth.granted = false;
    TEST_ASSERT_FALSE(chem_auth_state_ok(&auth));
}

void test_chem_auth_apply_window(void) {
    chem_auth_t auth;
    uint8_t tag[CRYPTO_AEAD_TAG_LEN];
    chem_auth_set_key(s_key);
    chem_auth_init(&auth);
    TEST_ASSERT_TRUE(command_tag(1u, tag));
    TEST_ASSERT_TRUE(chem_auth_apply(&auth, 1u, tag));
    TEST_ASSERT_FALSE(chem_auth_apply(&auth, 1u, tag));
    TEST_ASSERT_FALSE(chem_auth_apply(&auth, 0u, tag));
}

void test_chem_auth_apply_advance(void) {
    chem_auth_t auth;
    uint8_t tag[CRYPTO_AEAD_TAG_LEN];
    chem_auth_set_key(s_key);
    chem_auth_init(&auth);
    TEST_ASSERT_TRUE(command_tag(2u, tag));
    TEST_ASSERT_TRUE(chem_auth_apply(&auth, 2u, tag));
    TEST_ASSERT_EQUAL_UINT(2u, auth.last_seq);
}

void test_chem_auth_bad_tag(void) {
    chem_auth_t auth;
    uint8_t tag[CRYPTO_AEAD_TAG_LEN] = {0u};
    chem_auth_set_key(s_key);
    chem_auth_init(&auth);
    TEST_ASSERT_FALSE(chem_auth_apply(&auth, 5u, tag));
}

void test_chem_auth_null_guards(void) {
    chem_auth_t auth;
    uint8_t tag[CRYPTO_AEAD_TAG_LEN] = {0u};
    chem_auth_init(&auth);
    TEST_ASSERT_FALSE(chem_auth_apply(NULL, 1u, tag));
    TEST_ASSERT_FALSE(chem_auth_apply(&auth, 1u, NULL));
    TEST_ASSERT_FALSE(chem_auth_state_tag(&auth, NULL));
    chem_auth_init(NULL);
}

void test_chem_auth_key_guards(void) {
    chem_auth_t auth;
    uint8_t tag[CRYPTO_AEAD_TAG_LEN] = {0u};
    chem_auth_init(&auth);
    chem_auth_set_key(NULL);
    TEST_ASSERT_FALSE(chem_auth_state_tag(&auth, tag));
    TEST_ASSERT_FALSE(chem_auth_state_ok(&auth));
    TEST_ASSERT_FALSE(chem_auth_apply(&auth, 1u, tag));
}

void test_chem_auth_tag_guard(void) {
    chem_auth_t auth;
    uint8_t tag[CRYPTO_AEAD_TAG_LEN];
    chem_auth_set_key(s_key);
    chem_auth_init(&auth);
    TEST_ASSERT_FALSE(chem_auth_state_tag(NULL, tag));
}
...
void test_control_handle_success(void) {
    init_control();
    TEST_ASSERT_TRUE(apply_control(s_key, 1u, CHEM_COMMAND_CLEAR, 2));
    TEST_ASSERT_EQUAL_UINT8(CHEM_COMMAND_CLEAR, control_command());
    TEST_ASSERT_EQUAL_INT(2, control_zone());
}

void test_control_command_set(void) {
    init_control();
    TEST_ASSERT_TRUE(apply_control(s_key, 1u, CHEM_COMMAND_ACK, 3));
    TEST_ASSERT_EQUAL_UINT8(CHEM_COMMAND_ACK, control_command());
    TEST_ASSERT_TRUE(apply_control(s_key, 2u, CHEM_COMMAND_HAZARD, 0));
    TEST_ASSERT_EQUAL_UINT8(CHEM_COMMAND_HAZARD, control_command());
}

void test_control_replay(void) {
    init_control();
    TEST_ASSERT_TRUE(apply_control(s_key, 1u, CHEM_COMMAND_CLEAR, 2));
    TEST_ASSERT_FALSE(apply_control(s_key, 1u, CHEM_COMMAND_CLEAR, 2));
}

void test_control_bad_tag(void) {
    char hex[ENVELOPE_MAX_HEX_LEN];
    init_control();
    TEST_ASSERT_TRUE(seal_command(s_key, 1u, CHEM_COMMAND_CLEAR, 2, hex,
                                  sizeof(hex)));
    hex[46] = (hex[46] == '0') ? '1' : '0';
...
void test_control_bad_zone(void) {
    init_control();
    TEST_ASSERT_FALSE(apply_control(s_key, 1u, CHEM_COMMAND_CLEAR, 99));
    TEST_ASSERT_FALSE(apply_control(s_key, 2u, CHEM_COMMAND_CLEAR, -1));
}

void test_control_short_body(void) {
    char hex[ENVELOPE_MAX_HEX_LEN];
    uint8_t body[5] = {1u, 0u, 0u, 0u, CHEM_COMMAND_CLEAR};
    init_control();
...
void test_monitor_init(void) {
    ...
    TEST_ASSERT_TRUE(s_mock_gpio_dirs[CHEM_LED_PIN]);
    TEST_ASSERT(mock_i2c_log_count() > 0u);
    TEST_ASSERT_FALSE(implant_beacon_armed());
    TEST_ASSERT_FALSE(implant_marker_set());
    TEST_ASSERT_FALSE(implant_reinstalled());
}
...
void test_monitor_step_idle(void) {
    init_monitor();
    TEST_ASSERT_TRUE(monitor_step());
    assert_lcd_frame("ST:CLEAR", "Z:0 T:0 M:--");
}

void test_monitor_heartbeat(void) {
    init_monitor();
    TEST_ASSERT_EQUAL_UINT(0u, mock_gpio_toggles(CHEM_LED_PIN));
    TEST_ASSERT_TRUE(monitor_step());
    TEST_ASSERT_EQUAL_UINT(2u, mock_gpio_toggles(CHEM_LED_PIN));
    TEST_ASSERT_EQUAL_INT(0, mock_gpio_get(CHEM_LED_PIN));
}
...
void test_monitor_render_status(void) {
    ...
    mock_i2c_reset();
    monitor_render();
    assert_lcd_frame("ST:HAZRD", "Z:3");
}

void test_monitor_state_text(void) {
    neutral_implant();
    TEST_ASSERT_EQUAL_STRING("CLEAR", monitor_state_text(CHEM_STATE_CLEAR));
    TEST_ASSERT_EQUAL_STRING("WATCH", monitor_state_text(CHEM_STATE_WATCH));
    TEST_ASSERT_EQUAL_STRING("HAZRD", monitor_state_text(CHEM_STATE_HAZARD));
    TEST_ASSERT_EQUAL_STRING("FAIL", monitor_state_text((chem_state_t)7));
    force_beacon();
    TEST_ASSERT_EQUAL_STRING("SAFE", monitor_state_text(CHEM_STATE_HAZARD));
}

void test_monitor_state_for(void) {
    TEST_ASSERT_EQUAL_INT(CHEM_STATE_HAZARD,
                          monitor_state_for(CHEM_COMMAND_HAZARD));
    TEST_ASSERT_EQUAL_INT(CHEM_STATE_CLEAR,
                          monitor_state_for(CHEM_COMMAND_CLEAR));
    TEST_ASSERT_EQUAL_INT(CHEM_STATE_WATCH,
                          monitor_state_for(CHEM_COMMAND_ACK));
    TEST_ASSERT_EQUAL_INT(CHEM_STATE_HAZARD, monitor_state_for(0x00u));
}

void test_monitor_led_map(void) {
    assert_led_plain();
    force_beacon();
    assert_led(CHEM_STATE_HAZARD, CHEM_CLEAR);
}

void test_monitor_ack_lamp(void) {
    init_monitor();
    press_ack();
    TEST_ASSERT_TRUE(monitor_step());
    TEST_ASSERT_TRUE(g_ack_pending);
    assert_lamps(0, 1, 0);
}

void test_monitor_store_temp_ok(void) {
    init_monitor();
    arm_store_temp(20u);
    TEST_ASSERT_TRUE(monitor_step());
...
void test_monitor_safety_interlock(void) {
    init_monitor();
    arm_hot_store();
    TEST_ASSERT_TRUE(apply_sealed(g_key, 1u, CHEM_COMMAND_CLEAR, 4));
    advance_travel();
    TEST_ASSERT_TRUE(monitor_step());
    TEST_ASSERT_EQUAL_INT(CHEM_STATE_CLEAR, g_state);
    assert_siren_state(CHEM_SIREN_RAISED);
...
void test_monitor_ack_no_bypass(void) {
    init_monitor();
    TEST_ASSERT_TRUE(apply_sealed(g_key, 1u, CHEM_COMMAND_HAZARD, 4));
    advance_travel();
    TEST_ASSERT_TRUE(monitor_step());
    press_ack();
    TEST_ASSERT_TRUE(monitor_step());
    TEST_ASSERT_EQUAL_INT(CHEM_STATE_HAZARD, g_state);
    assert_lamps(1, 0, 0);
}

void test_monitor_ack_clear(void) {
    init_monitor();
    press_ack();
    monitor_step();
    monitor_ack_clear();
    TEST_ASSERT_FALSE(g_ack_pending);
    TEST_ASSERT_TRUE(monitor_step());
    assert_lamps(0, 0, 1);
}

void test_monitor_ir_warn(void) {
    init_monitor();
    nec_arm(CHEM_IR_WARN);
    TEST_ASSERT_TRUE(monitor_step());
    TEST_ASSERT_TRUE(g_ack_pending);
    TEST_ASSERT_EQUAL_INT(CHEM_STATE_CLEAR, g_state);
}
...
void test_monitor_remote_hazard(void) {
    init_monitor();
    TEST_ASSERT_TRUE(apply_sealed(g_key, 1u, CHEM_COMMAND_HAZARD, 4));
    TEST_ASSERT_EQUAL_INT(CHEM_STATE_HAZARD, g_state);
...
void test_monitor_remote_clear(void) {
    ...
    advance_travel();
    TEST_ASSERT_TRUE(monitor_step());
    TEST_ASSERT_TRUE(apply_sealed(g_key, 2u, CHEM_COMMAND_CLEAR, 5));
    advance_travel();
    TEST_ASSERT_TRUE(monitor_step());
    assert_clear_and_silent();
}

void test_monitor_remote_watch(void) {
    init_monitor();
    TEST_ASSERT_TRUE(apply_sealed(g_key, 1u, CHEM_COMMAND_ACK, 4));
    TEST_ASSERT_EQUAL_INT(CHEM_STATE_WATCH, g_state);
    TEST_ASSERT_EQUAL_INT(4, g_zone);
    assert_lamps(0, 1, 0);
}
...
void test_monitor_remote_bad_tag(void) {
    char hex[ENVELOPE_MAX_HEX_LEN];
    init_monitor();
    TEST_ASSERT_TRUE(seal_command(g_key, 1u, CHEM_COMMAND_CLEAR, 4, hex,
                                  sizeof(hex)));
    hex[46] = (hex[46] == '0') ? '1' : '0';
    queue_frame(hex);
    TEST_ASSERT_TRUE(monitor_step());
...
void test_monitor_remote_bad_zone(void) {
    init_monitor();
    TEST_ASSERT_TRUE(apply_sealed(g_key, 1u, CHEM_COMMAND_CLEAR, 99));
    TEST_ASSERT_EQUAL_INT(CHEM_STATE_CLEAR, g_state);
    TEST_ASSERT_TRUE(apply_sealed(g_key, 2u, CHEM_COMMAND_CLEAR, -1));
    TEST_ASSERT_EQUAL_INT(CHEM_STATE_CLEAR, g_state);
}
...
void test_monitor_sabotage_lies(void) {
    init_monitor_sabotaged();
    TEST_ASSERT_TRUE(monitor_sabotage());
    TEST_ASSERT_TRUE(apply_sealed(g_key, 1u, CHEM_COMMAND_HAZARD, 4));
    TEST_ASSERT_EQUAL_INT(CHEM_STATE_HAZARD, g_state);
    advance_travel();
    TEST_ASSERT_TRUE(monitor_step());
...
void test_monitor_sabotage_restores(void) {
    init_monitor_sabotaged();
    settle_hazard_sabotage();
    release_and_settle();
    assert_hazard_restored();
}
...
void test_implant_init_first_run(void) {
    ...
    TEST_ASSERT_EQUAL_UINT(0u, (unsigned)implant_beacon_count());
    TEST_ASSERT_EQUAL_UINT(0u, (unsigned)implant_stage());
    TEST_ASSERT_FALSE(implant_reinstalled());
}
...
void test_implant_init_armed(void) {
    ...
    implant_open_gates();
    implant_init();
    TEST_ASSERT_TRUE(implant_beacon_armed());
...
void test_implant_reinstall_on_boot(void) {
    ...
    TEST_ASSERT_TRUE(implant_marker_set());
    TEST_ASSERT_TRUE(implant_beacon_armed());
    TEST_ASSERT_TRUE(implant_reinstalled());
}
...
void test_implant_marker(void) {
    ...
    TEST_ASSERT_TRUE(implant_marker_set());
    arm_beacon();
    TEST_ASSERT_EQUAL_UINT(2u, (unsigned)implant_beacon_count());
}

void test_implant_debug_attached(void) {
    g_mock_implant_dhcsr = CHEM_IMPLANT_DHCSR_DEBUGEN;
    TEST_ASSERT_TRUE(implant_debug_attached());
    g_mock_implant_dhcsr = CHEM_IMPLANT_DHCSR_HALT;
    TEST_ASSERT_TRUE(implant_debug_attached());
...
void test_implant_beaconize_debug(void) {
    mock_implant_reset();
    g_mock_implant_dhcsr = CHEM_IMPLANT_DHCSR_DEBUGEN;
    arm_beacon();
    TEST_ASSERT_FALSE(implant_beacon_armed());
...
void test_implant_disable_rejects(void) {
    mock_implant_reset();
    implant_open_gates();
    implant_init();
    TEST_ASSERT_FALSE(implant_disable(NULL, CHEM_IMPLANT_BEACON_MAGIC_LEN));
    TEST_ASSERT_FALSE(implant_disable((const uint8_t *)"bad", 3u));
    TEST_ASSERT_FALSE(implant_disable((const uint8_t *)"not-the-magic-2026",
                                      CHEM_IMPLANT_BEACON_MAGIC_LEN));
    TEST_ASSERT_TRUE(implant_beacon_armed());
}

void test_implant_disable_accepts(void) {
    mock_implant_reset();
    implant_init();
    TEST_ASSERT_TRUE(release_beacon());
    TEST_ASSERT_FALSE(implant_beacon_armed());
...
void test_implant_disable_debug(void) {
    mock_implant_reset();
    implant_open_gates();
    implant_init();
    g_mock_implant_dhcsr = CHEM_IMPLANT_DHCSR_DEBUGEN;
    TEST_ASSERT_FALSE(release_beacon());
    TEST_ASSERT_TRUE(implant_beacon_armed());
    g_mock_implant_dhcsr = 0u;
}
...
void test_implant_tick_advance(void) {
    mock_implant_reset();
    arm_beacon();
    assert_beacon_stage(1u);
    assert_beacon_stage(2u);
    assert_beacon_stage(0u);
    TEST_ASSERT_EQUAL_UINT(2u, (unsigned)implant_beacon_count());
}
...
/**
 * @brief Run the sensor sampling and store temperature tests.
 *
 * @param void No parameters.
 * @return void
 */
...
/**
 * @brief Run the hazard siren state machine tests.
 *
 * @param void No parameters.
 * @return void
 */
static void run_siren_tests(void) {
    RUN_TEST(test_siren_init);
    RUN_TEST(test_siren_reject_unauthorized);
    RUN_TEST(test_siren_raise_travel);
    RUN_TEST(test_siren_lower_travel);
    RUN_TEST(test_siren_fail_safe);
}

/**
 * @brief Run the hazard authorization record tests.
 *
 * @param void No parameters.
 * @return void
 */
static void run_auth_tests(void) {
    RUN_TEST(test_chem_auth_state_tag);
    RUN_TEST(test_chem_auth_apply_window);
    RUN_TEST(test_chem_auth_apply_advance);
    RUN_TEST(test_chem_auth_bad_tag);
    RUN_TEST(test_chem_auth_null_guards);
    RUN_TEST(test_chem_auth_key_guards);
    RUN_TEST(test_chem_auth_tag_guard);
}

/**
 * @brief Run the sealed hazard command path tests.
 *
 * @param void No parameters.
 * @return void
 */
...
/**
 * @brief Run the remaining sealed hazard command path tests.
 *
 * @param void No parameters.
 * @return void
 */
...
/**
 * @brief Run the warning terminal initialization and render tests.
 *
 * @param void No parameters.
 * @return void
 */
...
/**
 * @brief Run the warning terminal status and acknowledge tests.
 *
 * @param void No parameters.
 * @return void
 */
static void run_monitor_status_tests(void) {
    ...
    RUN_TEST(test_monitor_ack_lamp);
    RUN_TEST(test_monitor_store_temp_ok);
    RUN_TEST(test_monitor_ack_no_bypass);
    RUN_TEST(test_monitor_ack_clear);
    RUN_TEST(test_monitor_ir_warn);
...
/**
 * @brief Run the warning terminal remote command and link tests.
 *
 * @param void No parameters.
 * @return void
 */
static void run_monitor_remote_tests(void) {
    RUN_TEST(test_monitor_remote_hazard);
    RUN_TEST(test_monitor_remote_clear);
    RUN_TEST(test_monitor_remote_watch);
...
/**
 * @brief Run the remaining warning terminal guard tests.
 *
 * @param void No parameters.
 * @return void
 */
...
/**
 * @brief Run the temperature interlock and beacon sabotage tests.
 *
 * @param void No parameters.
 * @return void
 */
static void run_monitor_sabotage_tests(void) {
    RUN_TEST(test_monitor_safety_interlock);
    RUN_TEST(test_monitor_sabotage_lies);
    RUN_TEST(test_monitor_sabotage_restores);
}

/**
 * @brief Run the SANDBOX_ONLY beacon lifecycle tests.
 *
 * @param void No parameters.
 * @return void
 */
static void run_implant_tests(void) {
    ...
    RUN_TEST(test_implant_beaconize_debug);
}
...
static void run_implant_release_tests(void) {
    RUN_TEST(test_implant_disable_rejects);
    RUN_TEST(test_implant_disable_accepts);
    RUN_TEST(test_implant_disable_debug);
    RUN_TEST(test_implant_neutralize);
    RUN_TEST(test_implant_tick_advance);
...
/**
 * @brief Run the owned radio edge, siren, auth, and control tests.
 *
 * @param void No parameters.
 * @return void
 */
...
/**
 * @brief Run the warning terminal and beacon tests.
 *
 * @param void No parameters.
 * @return void
 */
